...
# 변수 담을 리스트 생성
f=open("students.txt","r",encoding="utf-8") # 파일 읽기
while True: # 반복문 실행하여 다차원 리스트 읽어오기
  line = f.readline() # 파일에 데이터 읽어오기 
  if not line: break # 읽을 데이터가 없으면 중지
  s = line.strip().split(",") # 데이터를 줄바꿈 및 ,를 끊어서 읽음
  for i in range(7):
    if i==1:continue
